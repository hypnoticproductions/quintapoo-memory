#!/usr/bin/env node
/**
 * Standalone MCP Server for Sally/Telnyx Integration
 * 
 * This server runs independently and exposes 6 tools via stdio:
 ...
 * 
 * Or add to Telnyx AI Assistant MCP configuration:
 * {
 *   "mcpServers": {
 *     "quintapoo-sally": {
 *       "command": "node",
 *       "args": ["/path/to/mcp-server-standalone.mjs"],
 *       "env": {
 *         "DATABASE_URL": "mysql://..."
 *       }
 *     }
...
// Schema definitions (inline for standalone)
...
const callLogs = mysqlTable("call_logs", {
  id: varchar("id", { length: 128 }).primaryKey(),
  ...
  actionRequired: int("action_required").default(0).notNull(),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
  ...
  try {
    switch (request.params.name) {
      ...
      case "get_client_tasks": {
        const { client_id } = request.params.arguments;
      ...
      case "get_task_status": {
        const { manus_task_id } = request.params.arguments;
      ...
      case "get_client_history": {
        const { client_id, limit = 5 } = request.params.arguments;
      ...
      case "search_context": {
        const { client_id, query } = request.params.arguments;

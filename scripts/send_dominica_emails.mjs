#!/usr/bin/env node
/**
 * Send Dominica tourism emails via Resend API
 */
...
<body>
  ...
  <div class="highlight">
    <ul>
      ...
      <li><strong>Answer guest questions</strong> about Dominica history, culture, attractions, safety</li>
  ...
  <h3>Why This Matters for Dominica Tourism</h3>
  <ul>
  ...
  <p>Visit <strong>www.dopa.buzz</strong> and try Sally WUKR. Ask her about Dominica attractions in any language. See how she responds. Then imagine this voice agent supporting your tour guides in the field.</p>
  ...
  <h3>Let's Talk</h3>
  <p>I'm offering <strong>5 free pilot programs</strong> to Dominica tour operators this month. No credit card. No commitment. Just test Sally WUKR with your team for 30 days and see if it increases bookings.</p>
...
async function main() {
  ...
  // Fetch Dominica leads
  const [leads] = await connection.execute(
    `SELECT id, name, email, company, phone
     FROM leads
     WHERE source = 'dominica_tourism'
     AND status = 'new'
     LIMIT 100`
  );
  
  console.log(`Found ${leads.length} Dominica leads to email via Resend`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`✅ Dominica campaign complete!`);
